...
- **Repository integration status (2026-08-31):** `master` has diverged from
  ...
  commit and push manually. Do not add the untracked `.Codex/` directory unless it
...
| 6.6 | Do not tag `v0.1.0` until 6.2 and 6.3 are clean and YA confirms 6.5. `.Codex/`, `cWise.Rcheck/`, and `cWise_*.tar.gz` are now ignored for future runs, but generated check artifacts remain tracked. Codex could not untrack them because Git cannot create `.git/index.lock`; once writable, run `git rm -r --cached cWise.Rcheck cWise_0.1.0.tar.gz`, verify that the files remain locally, and commit those removals before tagging. | YA | ☐ |
